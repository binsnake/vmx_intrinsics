...
public:
    ...
    {
        mcallarg_t* fa = &funcinfo->args.push_back();
        fa->t = mop_r;
        fa->r = reg;
        fa->type = type;
...
public:
    ...
    {
        // Load field operand first
        mreg_t field_reg = cdg.load_operand(1);

        helpercall_builder_t builder(cdg, "__vmread", tinfo_t(BT_INT64 | BTMT_USIGNED));
        builder.add_register_argument(tinfo_t(BT_INT64 | BTMT_USIGNED), field_reg);
        builder.emit();
...
public:
    ...
    virtual merror_t apply(codegen_t& cdg)
    {
        // Load operands first to ensure proper data flow
        mreg_t field_reg = cdg.load_operand(0);
        mreg_t value_reg = cdg.load_operand(1);

        helpercall_builder_t builder(cdg, "__vmx_vmwrite", tinfo_t(BT_INT8 | BTMT_UNSIGNED));
        builder.add_register_argument(tinfo_t(BT_INT64 | BTMT_UNSIGNED), field_reg);
        builder.add_register_argument(tinfo_t(BT_INT64 | BTMT_UNSIGNED), value_reg);
        builder.emit();
...
public:
    ...
    virtual merror_t apply(codegen_t& cdg)
    {
        ...
        // Load address operand first
        mreg_t addr_reg = hacky_get_operand_address(cdg, 0);

        helpercall_builder_t builder(cdg, name, tinfo_t(BT_INT8 | BTMT_UNSIGNED));
        // TODO: should be "unsigned __int64*"
        builder.add_register_argument(tinfo_t::get_stock(STI_PVOID), addr_reg);
        builder.emit();
        do_ms_vmx_intrinsic_return(builder);
...
struct plugin_ctx_t : public plugmod_t
{
    ~plugin_ctx_t()
    {
        // Uninstall filters in reverse order
...
plugin_t PLUGIN = {
    ...
    init, // initialize
    nullptr, // terminate (not used with PLUGIN_MULTI)
    nullptr, // run (not used with PLUGIN_MULTI)
    comment,
